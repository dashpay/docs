...
- :ref:`dashman installation <masternode-setup-install-dashman>`
...
.. _masternode-setup-install-dashman:
...
installing dashman dependencies::
...
Clone the dashman repository, set up the dependencies and link the
CLI::
...
- Owner private key
- Operator private key (generate new or use private key generated by dashman)
- Voting private key
...
key generated by ``dashman setup``, this information is already
configured for your masternode. If you generated your own BLS key pair,
edit the dashman configuration as follows::

  dashman config:set core.masternode.operator.privateKey <bls_private_key>
  dashman restart

At this point you can go back to your terminal window and monitor your
masternode by entering ``dashman status`` or using the **Get status**
function in DMT.
...
you can use the BLS key generated by the ``dashman setup`` command.
...
**These keys are NOT stored by the wallet or dashman and must be kept
