...
A Sybil attack is a type of security threat on the masternode network where a malicious actor tries to take over part of the masternode network.

Purpose
-------
In the context of Dash it means that a malicious actor is trying to own or control as many masternodes as possible. Through owning or controlling masternodes an attacker can track the related mixing (CoinJoin) transactions performed by masternodes. Technically speaking an attacker could set up a masternode and start listening to the masternode's network traffic to analyze the mixing activity (CoinJoin) happening on the masternodes under his control. In this way the attacker could theoretically see how the transaction inputs and outputs are reshuffled, increasing the chance to deanonymize PrivateSend transactions (to identify the sender address).

Let's have a look at what the probability is for an attacker to deanonymize a PrivateSend transaction.
...
+----------+-----------------------+--------------+------------+-----------+---------------+
| DASH/USD | Masternode Collateral | Total        | Masternode | Available | Exchange      |
|          | Requirement           | Supply       | Supply     | Supply    | Supply        |
+==========+=======================+==============+============+===========+===============+
| $114     |         1000          |  9,870,544   |  4,829,000 | 5,041,544 | +/- 1,000,000 |
+----------+-----------------------+--------------+------------+-----------+---------------+

**Malicious Actor Already Present in The Masternode Network:**

+-------------------+---------------+---------------------+---------------------+---------------------+
| Masternodes owned | Current total |      4 Rounds       |       8 Rounds      |      16 Rounds      |
...
+--------------------+---------------+---------------------+---------------------+---------------------+
| Masternodes bought | Current total |      4 Rounds       |       8 Rounds      |      16 Rounds      |
| by attacker        | masternodes   |                     |                     |                     |
+====================+===============+=====================+=====================+=====================+
